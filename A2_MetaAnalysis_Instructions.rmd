...
ggplot(d_pub) +
  ...
  geom_vline(xintercept = m_pub, color="black") +
  theme_minimal() +
  theme(plot.title = element_text(size = 18L, face = "bold"))
```
...
```
```{r}
#Make a population pyramid plot 
...
na <- Matrix_MetaAnalysis %>% 
  select(AGE_M_HC, AGE_M_SZ, MALE_SZ, FEMALE_SZ, MALE_HC, FEMALE_HC)

na <- lapply(na, function(x) as.numeric(as.character(x)))
na <- na.omit(na)
na <- as.data.frame(na)
na
...
SZ_male <- na %>% 
  select(AGE_M_SZ, MALE_SZ)

SZ_male <- SZ_male[!is.na(as.numeric(SZ_male$MALE_SZ)), ]      # Delete rows
SZ_male
  

SZ_male$AGE_M_SZ <- as.integer(SZ_male$AGE_M_SZ)
SZ_male$MALE_SZ <- as.integer(SZ_male$MALE_SZ)


SZ_male <- SZ_male %>%                        # Aggregate values in range
  mutate(ranges = cut(AGE_M_SZ,
                      seq(10, 50, 5))) %>% 
  group_by(ranges) %>% 
  dplyr::summarize(sum_male = sum(MALE_SZ)) %>% 
  as.data.frame()




SZ_female <- Matrix_MetaAnalysis %>% 
  select(AGE_M_SZ, FEMALE_SZ)
SZ_female <- SZ_female[!is.na(as.numeric(SZ_female$FEMALE_SZ)), ]      # Delete rows


SZ_female$AGE_M_SZ <- as.integer(SZ_female$AGE_M_SZ)
SZ_female$FEMALE_SZ <- as.integer(SZ_female$FEMALE_SZ)

SZ_female <- SZ_female %>%                        # Aggregate values in range
  mutate(ranges = cut(AGE_M_SZ,
                      seq(10, 50, 5))) %>% 
  group_by(ranges) %>% 
  dplyr::summarize(sum_female = sum(FEMALE_SZ)) %>% 
  as.data.frame()

SZ <- merge(SZ_male, SZ_female,by="ranges")
...
df
  ...
  n1 <- ggplot(df.melt, aes(x = Age, y = Population, fill = Gender)) + 
  geom_bar(subset = .(Gender == "Female"), stat = "identity") + 
  geom_bar(subset = .(Gender == "Male"), stat = "identity") + 
    scale_y_continuous("Count", breaks = seq(-300, 150, 50)) +
   #                  labels = paste0(as.character(c(seq(15, 0, -5), seq(5, 15, 5))))) + 
  coord_flip() + 
    scale_fill_brewer(palette = "Dark2",
                      guide = guide_legend(
                        title = "Participant age and gender, Schizophrenia")) +
    theme_minimal()
...
HC_male <- na %>% 
  select(AGE_M_HC, MALE_HC)

HC_male <- HC_male[!is.na(as.numeric(HC_male$MALE_HC)), ]      # Delete rows
HC_male
  

HC_male$AGE_M_HC <- as.integer(HC_male$AGE_M_HC)
HC_male$MALE_HC <- as.integer(HC_male$MALE_HC)


HC_male <- HC_male %>%                        # Aggregate values in range
  mutate(ranges = cut(AGE_M_HC,
                      seq(10, 50, 5))) %>% 
  group_by(ranges) %>% 
  dplyr::summarize(sum_male = sum(MALE_HC)) %>% 
  as.data.frame()




HC_female <- Matrix_MetaAnalysis %>% 
  select(AGE_M_HC, FEMALE_HC)
HC_female <- HC_female[!is.na(as.numeric(HC_female$FEMALE_HC)), ]      # Delete rows


HC_female$AGE_M_HC <- as.integer(HC_female$AGE_M_HC)
HC_female$FEMALE_HC <- as.integer(HC_female$FEMALE_HC)

HC_female <- HC_female %>%                        # Aggregate values in range
  mutate(ranges = cut(AGE_M_HC,
                      seq(10, 50, 5))) %>% 
  group_by(ranges) %>% 
  dplyr::summarize(sum_female = sum(FEMALE_HC)) %>% 
  as.data.frame()

HC <- merge(HC_male, HC_female,by="ranges")
...
df
  ...
  n2 <- ggplot(df.melt, aes(x = Age, y = Population, fill = Gender)) + 
  geom_bar(subset = .(Gender == "Female"), stat = "identity") + 
  geom_bar(subset = .(Gender == "Male"), stat = "identity") + 
    scale_y_continuous("Count", breaks = seq(-300, 150, 50)) +
   #                  labels = paste0(as.character(c(seq(15, 0, -5), seq(5, 15, 5))))) + 
  coord_flip() + 
    scale_fill_brewer(palette = "Dark2",
                      guide = guide_legend(
                        title = "Participant age and gender, HC")) +
    theme_minimal()
...
p1 <- hist(Matrix_MetaAnalysis$SAMPLE_SIZE_HC)
p2 <- hist(Matrix_MetaAnalysis$SAMPLE_SIZE_SZ)


mean(Matrix_MetaAnalysis$SAMPLE_SIZE_HC)
hc <- Matrix_MetaAnalysis[!is.na(as.numeric(Matrix_MetaAnalysis$SAMPLE_SIZE_HC)), ]      # Delete rows
mean(hc$SAMPLE_SIZE_HC)


p1 <- ggplot(hc, aes(x=SAMPLE_SIZE_HC,)) +
  geom_histogram(position="identity") + 
  geom_vline(aes(xintercept=mean(SAMPLE_SIZE_HC)), color="blue", 
             linetype="dashed")+
  scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9"))  +
                       labs(title="Sample sizes, HC")
p1


p2 <- ggplot(Matrix_MetaAnalysis, aes(x=SAMPLE_SIZE_SZ,)) +
  geom_histogram(position="identity") + 
  geom_vline(aes(xintercept=mean(SAMPLE_SIZE_SZ)), color="blue",
             linetype="dashed")+
  scale_color_brewer(palette="Dark2") +
    labs(title="Sample sizes, SZ")
